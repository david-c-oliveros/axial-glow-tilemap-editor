...
void World::AddSolidTile(olc::vi2d index, TileType tTileType)
{
    ...
    if (index.x >= vSize.x)
    {
        int idx = index.y * vSize.x + index.x;
        int iDeltaSize = index.x - vSize.x;
        int iOldSize = vSize.x;
        vSize.x += iDeltaSize;
        sMap.resize(sMap.length() + (iDeltaSize * vSize.y), '=');
//        sMap.insert(sMap.begin() + idx, cTile);
        int aobCount = 0;
        if (iDeltaSize != 0)
        {
            for (int i = iOldSize; i < sMap.length(); i+=vSize.x)
            {
                for (int j = i + 1; j < iDeltaSize + i; j++)
                {
                    if (j >= sMap.length())
                    {
                        std::cout << "Out of Bounds - " << ++aobCount << std::endl;
                        break;
                    }
                    sMap[j] = '+';
                }
